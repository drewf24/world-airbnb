...
NOTE
-- did not do final selection so that it would push to git 
-- we are getting neighborhood in our final selection and that is leading to an output with EVERY neighborhood in the table -- not a good idea since there are soooo many

To conclude our analysis we will look at the influence of all the variables on price. 

Removing NA values and creating a full model for price:

```{r remove-na-price}
abnb_model_price <- abnb_sample %>%
  drop_na(neighbourhood_group, neighbourhood, latitude, 
          longitude, room_type, minimum_nights, number_of_reviews,
          last_review, reviews_per_month, calculated_host_listings_count,
          availability_365, reviews_per_month)

m_full_model_price <- lm(price ~ neighbourhood_group +
                           ...
                           availability_365 + 
                           reviews_per_month, data = abnb_model_price)
```
...
The R squared value is . This means that  % of the variability in price can be explained using this model.
...
`availability_365-hat = 115.774 -3.443*(room_typePrivateroom) +74.912*(room_typeShared room)`
...
`availability_365-hat = 59.923 + 26.396*(room_typePrivate room) +115.98*(room_typeShared room) + 0.481*(minimum_nights) +0.461*(number_of_reviews) + 0.853*(calculated_host_listings_count) +29.622*(price_caseMedian or Above) + 5.93*(reviews_per_month)`
...
-- I COULD NOT INCLUDE NAME WITHOUT IT DYING ON ME

To conclude our analysis we will look at the influence of all the variables on availibility (not including host_id, name and host_name) 

Removing NA values and creating a full model for price:

```{r remove-na-avail}
abnb_model_availability <- abnb_sample %>%
  drop_na(neighbourhood_group, neighbourhood, latitude, 
          longitude, room_type, minimum_nights, number_of_reviews,
          last_review, reviews_per_month, calculated_host_listings_count,
          availability_365, reviews_per_month, price)

m_full_model_price <- lm(availability_365 ~ neighbourhood_group +
                           neighbourhood + latitude +
                           longitude + room_type + 
                           minimum_nights +  
                           number_of_reviews + last_review +
                           ...
                           calculated_host_listings_count + 
                           price + 
                           reviews_per_month, data = abnb_model_availability)
```

Performing model selection using AIC for availability using step function:

```{r AIC-backward-selection-availability, results="hide"}
selected_model_availability <- step(m_full_model_price, direction = "backward")

tidy(selected_model_availability) %>%
  select(term, estimate) %>%
  kable(format = "markdown", digits = 3)
```

The selected linear model by backward selection is:

`availability_365-hat = 21575.102 - 90.533*(latitude) + 260.527*(longitude) + 11.112*(room_typePrivate room) + 92.026*(room_typeShared room) + 0.453*(minimum_nights) + 0.364*(number_of_review) + 0.082(last_review) + 0.819(calculated_host_listinga_count) + 0.075*(price)`

```{r r-square-selectedmodel-avail}
glance(selected_model_availability)$r.squared
```

The R-squared value is 18.127% of the variability in availibility can be explained by the selected model. 
